...
- **Models:** **All required models are provided by `./assets/models.zip`** (SHA-256 `33d0a9194de3cbd667fd329ab7c6ce832f4e1373ba0b1844ce0040a191abc483`). Extract to `./models` and reuse the unpacked assets. Do **not** download alternate model files. This is a large almost 2 gig file it will take a minute to download and extract.
...
# Verify integrity (source of truth: models.zip.sha256 in repo)
# Expected SHA-256: 33d0a9194de3cbd667fd329ab7c6ce832f4e1373ba0b1844ce0040a191abc483
...
3. **All features exercised (Cloud Strict):** Quiet-Boost, background SED (ON), diarization, ASR, affect V/A/D, speech emotion 8-class, text emotions 28-class, intent. **VAD_dim is mandatory.**
   - Optional two-phase workflow (non-strict or dev): You may run `diaremot core` followed by `diaremot enrich` in the same job to produce equivalent artifacts. A “core-only” run does not satisfy strict gates until enrichment completes.
